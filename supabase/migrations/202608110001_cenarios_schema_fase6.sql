-- DEC-007 (Gerador e Comparador de Cenários Viáveis) - Fase 6: migration
-- de schema. Todo o núcleo de cálculo (Fases 0-5) já está implementado e
-- testado em TypeScript puro, sem tocar banco - esta migration só cria a
-- estrutura que os testes já provaram necessária (§18: "Fase 1+5 - schema
-- reflete o que os testes já provaram necessário").
--
-- ATENÇÃO - CORREÇÕES CONTRA O SCHEMA REAL (não o de supabase/baseline/,
-- que é uma reescrita paralela NUNCA aplicada ao projeto remoto -
-- confirmado no próprio supabase/baseline/README.md; usar baseline como
-- fonte de verdade teria produzido pelo menos 2 erros reais nesta
-- migration). Toda referência a coluna/tabela abaixo foi conferida
-- diretamente contra supabase/migrations/*.sql (cronológico) e, quando a
-- CREATE TABLE original não está rastreada nas migrations (recursos_produtivos,
-- grupos_recursos - pré-datam o histórico deste repositório), contra o uso
-- real em src/ (Supabase client) como segunda fonte:
--   - DEC-007 §14 diz "ordens_producao" - a tabela real é ordens_fabricacao.
--   - DEC-007 §14 lista uma tabela "operacoes_producao" (operação de
--     produção, granularidade abaixo de ordens_fabricacao) - CONFIRMADO
--     por preflight direto no remoto (information_schema.tables, filtro
--     por %oper%/%ordem%/%apont%/%aloc%) que ela NÃO EXISTE e não há
--     equivalente (só bom_operacoes - tabela de ROTEIRO, não de
--     execução -, calendario_operacional_empresa - já coberta em outro
--     item do inventário - e 2 views, que não recebem trigger de
--     escrita). O item 8.20 correspondente foi REMOVIDO do inventário
--     desta migration (não criar trigger para tabela inexistente) - ver
--     DEC-007 §17.1 para o registro completo da correção.
--   - DEC-007 §14 diz "recursos_produtivos.grupo_id" - CONFIRMADO correto
--     (não grupo_recurso_id, que só existe no schema paralelo de baseline).
--   - DEC-007 §14 diz "recursos_produtivos.capacidade_horas_dia" -
--     CONFIRMADO que existe (202607120001), carga_horaria_semanal/
--     dias_trabalhados_semana (202607140001) são aditivos, alimentam esse
--     valor, não o substituem.
--   - bom_itens.componente_tipo e bom_itens.ativo já existiam na
--     CREATE TABLE original (202606050032) - não foram adicionados depois.
--   - projetos.ativo e projeto_itens.ativo já existiam nas CREATE TABLE
--     originais (202606030001, 202606030002).
--   - projeto_itens: a FK para itens_industriais chama-se literalmente
--     produto_id (não item_industrial_id).
--   - unique(id, empresa_id) SÓ existe hoje (antes desta migration) em
--     recursos_produtivos (202607200001) e simulacao_comercial_itens
--     (202608020001) - bom_operacoes, bom_itens e
--     simulacao_comercial_item_distribuicoes NÃO têm essa constraint no
--     schema real (mesmo bom_itens tendo em baseline) - as 3 são
--     adicionadas nesta migration, pré-requisito das FKs compostas.
--   - 4 nomes de CHECK desta própria migration excediam o limite de 63
--     bytes do Postgres (NAMEDATALEN) e teriam sido truncados
--     silenciosamente na criação - detectado no teste 1 (estrutura) no
--     remoto, encurtados explicitamente (removido "_por" do meio, sem
--     perda de significado, todos ≤61 caracteres, checados um a um
--     contra colisão com os demais nomes do inventário):
--     simulacao_comercial_item_distribuicoes_referencia_origem_chk,
--     _contratacao_origem_chk, _capacidade_origem_chk (todos na mesma
--     tabela) e simulacao_comercial_item_distribuicao_dias_nat_contrato_chk.
--   - calendario_oficial_feriados NÃO tem empresa_id (tabela global,
--     mantida externamente) - o trigger de versão nela precisa
--     incrementar TODAS as empresas, não uma só (única exceção ao padrão
--     "incrementa via empresa_id da própria linha" usado em todo o resto
--     do inventário §14).
--
-- APLICAÇÃO NO BANCO REMOTO SOMENTE PELO SQL EDITOR, APÓS TESTE COMPLETO
-- EM BEGIN...ROLLBACK (scripts separados, não implantados - ver
-- supabase/tests/fase6_cenarios_teste_1_estrutura.sql,
-- _2_precedencia_versao.sql, _3_snapshots_imutabilidade.sql e
-- _4_integracao_rpc.sql, fora de supabase/migrations/ e fora do padrão
-- de nome de migration de propósito, para nunca serem aplicados por
-- engano via supabase db push). NÃO UTILIZAR `supabase db push`
-- (histórico de migrations dessincronizadas, mesmo motivo já registrado
-- nas migrations anteriores desta safra).
--
-- Migration inteira em UMA transação (DEC-007 §18: "uma única
-- transação") - ou todo o schema desta fase entra (tabela de versão,
-- backfill, RLS, triggers de versão E de imutabilidade), ou nada entra.
-- Nenhuma escrita de dado de negócio acontece aqui além do backfill
-- estrutural de empresa_capacidade_versoes (1 linha por empresa já
-- existente, versao=1 - não altera nenhum dado de cenário/simulação).
