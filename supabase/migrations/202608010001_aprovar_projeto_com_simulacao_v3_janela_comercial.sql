-- RPC v3 de aprovacao da Simulacao Comercial - Entrega 1 do PAD-008 v2.0
-- (secao 17: camada de preparacao comercial / DEC-004 v1.1). Adiciona
-- as duas premissas novas (Margem de Seguranca ja existia; Data
-- Prevista de Aprovacao do Pedido e nova) e a data derivada que ainda
-- nao tinha coluna propria (Data de Chegada Prevista). janela_inicio/
-- janela_fim SAO REAPROVEITADAS sem mudanca de schema: passam a
-- significar Data de Disponibilidade para Producao / Prazo Interno
-- (PAD-008 v2.0 secao 17), nunca mais digitadas manualmente.
--
-- Aditiva, mesmo padrao da transicao v1 -> v2 (202607300001): NAO
-- remove nem altera aprovar_projeto_com_simulacao_v2. A v2 continua
-- existindo, sem EXECUTE para authenticated (nunca teve), como caminho
-- de rollback. Distribuicao parcial entre recursos e calculador
-- reverso diario (PAD-008 v2.0 secoes 18-19) permanecem FORA do escopo
-- desta migration - recurso_considerado_id continua singular, itens
-- persistidos com a mesma granularidade e o mesmo formato da v2.
--
-- APLICACAO NO BANCO REMOTO SOMENTE PELO SQL EDITOR, APOS REVISAO
-- EXPLICITA. NAO UTILIZAR `supabase db push` devido ao historico
-- de migrations antigas dessincronizadas.
--
-- Migration inteira envolvida em transacao explicita (begin/commit) -
-- correcao da auditoria da Entrega 1, ponto 7: ou as colunas novas, a
-- function e os grants sao aplicados todos juntos, ou nada e aplicado.

begin;

-- =========================================================
-- Colunas novas em simulacoes_comerciais
-- =========================================================
alter table public.simulacoes_comerciais
  add column data_prevista_aprovacao_pedido date,
  add column data_chegada_prevista date;

comment on column public.simulacoes_comerciais.data_prevista_aprovacao_pedido is
  'Estimativa do orcamentista de quando o pedido do cliente sera formalmente confirmado (DEC-004 v1.1, "Papel do orcamentista"). Diferente de projetos.situacao_comercial (fato ja observado) - e uma previsao editavel, usada como data-base do calculo de disponibilidade de material (PAD-008 v2.0 secao 17.4). NULL em linhas anteriores a Entrega 1 (aprovadas sob a regra de janela manual) - nao retroativo.';

comment on column public.simulacoes_comerciais.data_chegada_prevista is
  'Data Prevista de Aprovacao do Pedido + 9 dias produtivos (regra provisoria de material, DEC-004 v1.1/PAD-008 v2.0 secao 17.4) - substituida pelo dado real de Compras quando o modulo existir, sem mudanca de schema. NULL em linhas anteriores a Entrega 1.';

comment on column public.simulacoes_comerciais.janela_inicio is
  'A partir da Entrega 1 (PAD-008 v2.0 secao 17): Data de Disponibilidade para Producao, derivada automaticamente (proximo dia produtivo apos data_chegada_prevista) - nao e mais digitada manualmente. Em linhas anteriores a Entrega 1, era a data inicial informada manualmente pelo orcamentista.';

comment on column public.simulacoes_comerciais.janela_fim is
  'A partir da Entrega 1 (PAD-008 v2.0 secao 17): Prazo Interno, derivado automaticamente (data_necessidade - margem_seguranca_dias em dias produtivos) - nao e mais digitado manualmente. Em linhas anteriores a Entrega 1, era a data final informada manualmente pelo orcamentista.';

-- =========================================================
-- aprovar_projeto_com_simulacao_v3
-- =========================================================
create or replace function public.aprovar_projeto_com_simulacao_v3(
  p_aprovado_por uuid,
  p_projeto_id uuid,
...
begin
  ...
  if p_data_prevista_aprovacao_pedido is null then
    raise exception 'p_data_prevista_aprovacao_pedido e obrigatoria (PAD-008 v2.0 secao 17) - premissa informada antes da simulacao, nunca omitida na aprovacao.';
  end if;

  if p_data_chegada_prevista is null then
    raise exception 'p_data_chegada_prevista e obrigatoria - deve vir do recalculo autoritativo no servidor (prepararJanelaComercial), nunca de valor derivado enviado pelo cliente.';
  end if;
  ...
  if p_janela_fim < p_janela_inicio then
    raise exception 'p_janela_fim (%) nao pode ser anterior a p_janela_inicio (%) - ausencia de janela produtiva deve ser barrada antes de chamar esta RPC.', p_janela_fim, p_janela_inicio;
  end if;
  ...
  -- mais abaixo - esta checagem so acelera o caminho feliz (chave
  -- repetida, sem corrida) para nao repetir toda a validacao de itens
  -- a toa.
  ...
  -- v2, reaproveitada sem alteracao: distribuicao parcial entre
  -- recursos (PAD-008 v2.0 secao 19) e' decisao aprovada, PENDENTE DE
  -- IMPLEMENTACAO - fora do escopo desta migration. recurso_considerado_id
  -- continua singular.
  ...
  -- Insercao atomica com tratamento de conflito - mesma logica da v2
  -- (ver comentario detalhado em 202607300001). Insere com
  -- vigente=false de proposito; so vira vigente depois de confirmar
  -- que VENCEMOS a insercao.
  ...
  -- exige (ver 202607190006) - v3 precisa seta-la tambem, e' uma
  -- function diferente da v1/v2 aos olhos da trigger.
...
comment on function public.aprovar_projeto_com_simulacao_v3(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text) is
  'Versao 3 da aprovacao da Simulacao Comercial (PAD-008 v2.0, secao 17 - Entrega 1) - so chamavel por service_role, atraves de um client privilegiado server-only. Estende a v2 com as premissas comerciais da janela derivada automaticamente: p_data_prevista_aprovacao_pedido (nova) e p_data_chegada_prevista (nova, calculada no servidor via prepararJanelaComercial). p_janela_inicio/p_janela_fim sao reaproveitadas: passam a significar Data de Disponibilidade para Producao / Prazo Interno, nunca mais digitadas manualmente. Resolve empresa a partir de public.usuarios do aprovador (p_aprovado_por), nunca de parametro do chamador. Idempotente por (empresa_id, p_chave_idempotencia, p_hash_solicitacao): mesma chave e mesmo hash retornam o snapshot ja gravado; mesma chave com hash diferente e conflito, rejeitado. Mesma validacao estrutural e mesma atomicidade da v2. Distribuicao parcial entre recursos (PAD-008 v2.0 secao 19) e calculador reverso diario (secao 18) permanecem FORA do escopo desta funcao - recurso_considerado_id continua singular. Nao recalcula os valores numericos dos itens nem a janela recebidos - a garantia de que vieram de uma execucao/preparacao real e responsabilidade de quem chama esta function (Server Action, que recalcula ambos no servidor antes de chamar), nao desta RPC.';
...
-- funcionar.
revoke execute on function public.aprovar_projeto_com_simulacao_v3(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text) from public;
revoke execute on function public.aprovar_projeto_com_simulacao_v3(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text) from anon;
revoke execute on function public.aprovar_projeto_com_simulacao_v3(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text) from authenticated;
grant execute on function public.aprovar_projeto_com_simulacao_v3(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text) to service_role;
