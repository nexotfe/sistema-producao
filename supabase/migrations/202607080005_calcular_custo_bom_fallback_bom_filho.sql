-- Corrige bug real: calcular_custo_bom so buscava o bom do produto-filho
-- (subconjunto) com status='ativo', SEM fallback. Como todo roteiro novo
-- e criado com status='rascunho' (criarPrimeiroRoteiro()) e nao existe
-- nenhuma tela para promover um bom a 'ativo', a esmagadora maioria dos
-- produtos-filho reais fica presa em 'rascunho' para sempre - fazendo o
-- subconjunto contribuir SEMPRE 0 no custo do produto pai, mesmo com
-- materia-prima/operacoes reais cadastradas no filho.
--
-- Corrigido para espelhar EXATAMENTE a mesma logica de escolha de bom ja
-- usada no frontend (useRoteiro.ts -> carregarBom): prefere o bom
-- status='ativo'; se nao houver nenhum ativo, cai para o bom mais
-- recente (created_at desc) independente do status.
...
comment on function public.calcular_custo_bom(uuid, integer) is
  'Custo industrial de um bom por categoria (materia_prima, subconjunto, engenharia, mao_de_obra, terceiros, logistica, total). Materia-prima converte a unidade de consumo para a unidade de preco antes de multiplicar. Subconjunto e recursivo (custo do bom do produto-filho x quantidade); bom do filho preferido = ativo, senao o mais recente (mesma regra de useRoteiro.ts).';
