-- TESTE 3/4 da migration 202608110001_cenarios_schema_fase6.sql -
-- SNAPSHOTS E IMUTABILIDADE. NÃO É UMA MIGRATION - vive em supabase/tests/, fora do
-- padrão de nome de migration, NUNCA deve ser aplicado em produção.
--
-- Escopo deste script: marca transacional de vigente, imutabilidade de conteúdo do snapshot, append-only nas 3 tabelas filhas, CHECK de capacidade por origem (incl. TERCEIRIZADO), regras de dia (natureza×contratacao_id, rejeição de dia com pai TERCEIRIZADO) e coerência de ajuste_cenario/custo_adicional_total.
--
...
-- =====================================================================
-- PARTE 2: CENÁRIOS DE TESTE - SNAPSHOTS E IMUTABILIDADE
-- =====================================================================

-- ---------------------------------------------------------------------
-- TESTE 1: UPDATE direto de vigente é rejeitado SEM a marca transacional
-- ---------------------------------------------------------------------
do $$
declare
  v_id uuid;
begin
  select id into v_id from public.simulacoes_comerciais where vigente = true limit 1;
  if v_id is null then
    raise notice 'TESTE 1 PULADO: nenhuma simulacoes_comerciais com vigente=true no ambiente.';
  else
    begin
      update public.simulacoes_comerciais set vigente = false where id = v_id;
      raise exception 'TESTE 1 FALHOU: UPDATE direto de vigente sem marca deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 1 FALHOU%' then raise; end if;
        raise notice 'TESTE 1 OK: UPDATE direto de vigente sem marca rejeitado (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 2: UPDATE de vigente é rejeitado tentando REATIVAR
-- (false→true) MESMO COM a marca setada para o projeto certo
-- ---------------------------------------------------------------------
do $$
declare
  v_id uuid;
  v_projeto_id uuid;
begin
  select id, projeto_id into v_id, v_projeto_id
  from public.simulacoes_comerciais where vigente = false limit 1;
  if v_id is null then
    raise notice 'TESTE 2 PULADO: nenhuma simulacoes_comerciais com vigente=false no ambiente.';
  else
    begin
      perform set_config('app.aprovacao_em_andamento_projeto_id', v_projeto_id::text, true);
      update public.simulacoes_comerciais set vigente = true where id = v_id;
      raise exception 'TESTE 2 FALHOU: reativação direta (false→true) deveria ter sido rejeitada mesmo com marca.';
    exception
      when others then
        if sqlerrm like 'TESTE 2 FALHOU%' then raise; end if;
        raise notice 'TESTE 2 OK: reativação rejeitada mesmo com marca (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 3: comparação de imutabilidade (to_jsonb) cobre uma coluna NOVA
-- adicionada DE PROPÓSITO neste teste, SEM editar o trigger
-- ---------------------------------------------------------------------
do $$
declare
  v_id uuid;
begin
  alter table public.simulacoes_comerciais add column teste_coluna_nova_fase6 text;

  select id into v_id from public.simulacoes_comerciais limit 1;
  if v_id is null then
    raise notice 'TESTE 3 PULADO: nenhuma simulacoes_comerciais no ambiente.';
  else
    begin
      update public.simulacoes_comerciais set teste_coluna_nova_fase6 = 'qualquer-valor' where id = v_id;
      raise exception 'TESTE 3 FALHOU: UPDATE de coluna nova (fora de vigente) deveria ter sido rejeitado pela comparação to_jsonb do conteúdo.';
    exception
      when others then
        if sqlerrm like 'TESTE 3 FALHOU%' then raise; end if;
        raise notice 'TESTE 3 OK: to_jsonb cobriu a coluna nova automaticamente, sem editar o trigger (%).', sqlerrm;
    end;
  end if;

  alter table public.simulacoes_comerciais drop column teste_coluna_nova_fase6;
end;
$$;


-- ---------------------------------------------------------------------
-- TESTE 4: UPDATE/DELETE rejeitado nas 3 tabelas filhas append-only
-- (4a/4b aqui; 4c - simulacao_comercial_item_distribuicao_dias - fica
-- adiante, depois do TESTE 7, que é quem insere uma linha real nessa
-- tabela para o teste ter fixture)
-- ---------------------------------------------------------------------
do $$
declare
  v_id uuid;
begin
  select id into v_id from public.simulacao_comercial_itens limit 1;
  if v_id is null then
    raise notice 'TESTE 4a PULADO: nenhuma simulacao_comercial_itens no ambiente.';
  else
    begin
      update public.simulacao_comercial_itens set necessario = necessario where id = v_id;
      raise exception 'TESTE 4a FALHOU: UPDATE em simulacao_comercial_itens deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 4a FALHOU%' then raise; end if;
        raise notice 'TESTE 4a OK: UPDATE em simulacao_comercial_itens rejeitado (%).', sqlerrm;
    end;
  end if;

  select id into v_id from public.simulacao_comercial_item_distribuicoes limit 1;
  if v_id is null then
    raise notice 'TESTE 4b PULADO: nenhuma simulacao_comercial_item_distribuicoes no ambiente.';
  else
    begin
      delete from public.simulacao_comercial_item_distribuicoes where id = v_id;
      raise exception 'TESTE 4b FALHOU: DELETE em simulacao_comercial_item_distribuicoes deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 4b FALHOU%' then raise; end if;
        raise notice 'TESTE 4b OK: DELETE em simulacao_comercial_item_distribuicoes rejeitado (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 6: CHECK de capacidade-por-origem em simulacao_comercial_item_distribuicoes
-- ---------------------------------------------------------------------
do $$
declare
  v_item_id uuid;
  v_empresa_id uuid;
begin
  select id, empresa_id into v_item_id, v_empresa_id from public.simulacao_comercial_itens limit 1;
  if v_item_id is null then
    raise notice 'TESTE 6 PULADO: nenhuma simulacao_comercial_itens no ambiente para ancorar a distribuição de teste.';
  else
    begin
      -- TERCEIRIZADO com capacidade preenchida (inválido) - esperado falhar.
      insert into public.simulacao_comercial_item_distribuicoes (
        empresa_id, simulacao_comercial_item_id, recurso_id, ordem_consideracao, origem,
        capacidade_bruta_periodo, produtividade_considerada, capacidade_efetiva,
        comprometido_inicial, capacidade_disponivel_inicial, capacidade_disponivel_antes,
        horas_padrao_alocadas, horas_maquina_estimadas, capacidade_disponivel_depois,
        recurso_externo_tipo, recurso_externo_nome, contratacao_id, custo,
        data_inicio_calculada, data_fim_calculada
      ) values (
        v_empresa_id, v_item_id, null, 0, 'TERCEIRIZADO',
        10, 1, 10, 0, 10, 10, 4, 4, 6, -- capacidade preenchida - deve violar o CHECK
        'terceirizacao', 'Fornecedor Teste', 'CT-TESTE', 100,
        '2026-11-09', '2026-11-11'
      );
      raise exception 'TESTE 6 FALHOU: TERCEIRIZADO com capacidade preenchida deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 6 FALHOU%' then raise; end if;
        raise notice 'TESTE 6 OK: TERCEIRIZADO com capacidade preenchida rejeitado (%).', sqlerrm;
    end;

    begin
      -- TERCEIRIZADO coerente (sem capacidade, com datas/custo/contrato) - esperado passar.
      insert into public.simulacao_comercial_item_distribuicoes (
        empresa_id, simulacao_comercial_item_id, recurso_id, ordem_consideracao, origem,
        recurso_externo_tipo, recurso_externo_nome, contratacao_id, custo,
        data_inicio_calculada, data_fim_calculada
      ...
      raise notice 'TESTE 6b OK: TERCEIRIZADO coerente (sem capacidade) aceito normalmente.';
    exception
      when others then
        raise exception 'TESTE 6b FALHOU: TERCEIRIZADO coerente deveria ter sido aceito - %.', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 7: simulacao_comercial_item_distribuicao_dias - natureza='normal' ⇔ contratacao_id nulo
-- ---------------------------------------------------------------------
do $$
declare
  v_dist_id uuid;
  v_empresa_id uuid;
begin
  select id, empresa_id into v_dist_id, v_empresa_id from public.simulacao_comercial_item_distribuicoes limit 1;
  if v_dist_id is null then
    raise notice 'TESTE 7 PULADO: nenhuma simulacao_comercial_item_distribuicoes no ambiente (nem a inserida no TESTE 6, se ele foi pulado).';
  else
    begin
      insert into public.simulacao_comercial_item_distribuicao_dias
        (empresa_id, simulacao_comercial_item_distribuicao_id, data, horas_maquina, horas_padrao, natureza, contratacao_id)
      values (v_empresa_id, v_dist_id, '2026-11-09', 4, 4, 'normal', 'CT-INDEVIDO');
      raise exception 'TESTE 7a FALHOU: natureza=normal com contratacao_id preenchido deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 7a FALHOU%' then raise; end if;
        raise notice 'TESTE 7a OK: natureza=normal com contratacao_id rejeitado (%).', sqlerrm;
    end;

    begin
      insert into public.simulacao_comercial_item_distribuicao_dias
        (empresa_id, simulacao_comercial_item_distribuicao_id, data, horas_maquina, horas_padrao, natureza, contratacao_id)
      values (v_empresa_id, v_dist_id, '2026-11-10', 4, 4, 'hora_extra', null);
      raise exception 'TESTE 7b FALHOU: natureza=hora_extra sem contratacao_id deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 7b FALHOU%' then raise; end if;
        raise notice 'TESTE 7b OK: natureza=hora_extra sem contratacao_id rejeitado (%).', sqlerrm;
    end;

    -- Linha VÁLIDA (natureza=hora_extra COM contratacao_id) - fica gravada
    -- de propósito para o TESTE 4c (abaixo) ter fixture real de append-only
    -- nesta tabela, sem depender de dado pré-existente no ambiente.
    insert into public.simulacao_comercial_item_distribuicao_dias
      (empresa_id, simulacao_comercial_item_distribuicao_id, data, horas_maquina, horas_padrao, natureza, contratacao_id)
    values (v_empresa_id, v_dist_id, '2026-11-11', 4, 4, 'hora_extra', 'CT-TESTE-7C');
    raise notice 'TESTE 7c OK: linha válida (hora_extra + contratacao_id) inserida - servirá de fixture para o TESTE 4c.';
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 7d: TERCEIRIZADO não aceita linha diária (NOVO - auditoria
-- apontou que o modelo diz que TERCEIRIZADO opera só por datas
-- calculadas, sem alocação diária, mas a tabela de dias aceitava
-- qualquer distribuição pai, inclusive terceirizada, sem validação).
-- Usa a distribuição TERCEIRIZADA inserida pelo TESTE 6b.
-- ---------------------------------------------------------------------
do $$
declare
  v_dist_terceirizada_id uuid;
  v_empresa_id uuid;
begin
  select id, empresa_id into v_dist_terceirizada_id, v_empresa_id
  from public.simulacao_comercial_item_distribuicoes
  where origem = 'TERCEIRIZADO'
  limit 1;

  if v_dist_terceirizada_id is null then
    raise notice 'TESTE 7d PULADO: nenhuma distribuição TERCEIRIZADO no ambiente (nem a inserida no TESTE 6b, se ele foi pulado).';
  else
    begin
      insert into public.simulacao_comercial_item_distribuicao_dias
        (empresa_id, simulacao_comercial_item_distribuicao_id, data, horas_maquina, horas_padrao, natureza, contratacao_id)
      values (v_empresa_id, v_dist_terceirizada_id, '2026-11-09', 4, 4, 'normal', null);
      raise exception 'TESTE 7d FALHOU: linha diária com pai origem=TERCEIRIZADO deveria ter sido rejeitada.';
    exception
      when others then
        if sqlerrm like 'TESTE 7d FALHOU%' then raise; end if;
        raise notice 'TESTE 7d OK: linha diária com pai TERCEIRIZADO rejeitada (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 4c: UPDATE/DELETE rejeitado em simulacao_comercial_item_distribuicao_dias
-- (a 3ª tabela filha append-only - auditoria apontou que o TESTE 4
-- original só cobria 2 das 3; roda aqui, depois do TESTE 7, porque só
-- então existe uma linha real nesta tabela para servir de fixture)
-- ---------------------------------------------------------------------
do $$
declare
  v_id uuid;
begin
  select id into v_id from public.simulacao_comercial_item_distribuicao_dias limit 1;
  if v_id is null then
    raise notice 'TESTE 4c PULADO: nenhuma simulacao_comercial_item_distribuicao_dias no ambiente (nem a inserida no TESTE 7).';
  else
    begin
      delete from public.simulacao_comercial_item_distribuicao_dias where id = v_id;
      raise exception 'TESTE 4c FALHOU: DELETE em simulacao_comercial_item_distribuicao_dias deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 4c FALHOU%' then raise; end if;
        raise notice 'TESTE 4c OK: DELETE em simulacao_comercial_item_distribuicao_dias rejeitado (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 8: ajuste_cenario / custo_adicional_total coerência
-- (corrigido pela auditoria: o teste original fazia UPDATE numa linha
-- existente, que é barrado PRIMEIRO pela imutabilidade de conteúdo
-- (impedir_alteracao_conteudo_snapshot, que reage a QUALQUER UPDATE,
-- não só vigente) - nunca chegava a exercitar a CHECK que dizia testar.
-- Agora testa via INSERT direto (a imutabilidade só se aplica a
-- UPDATE), e confirma que o erro capturado é especificamente da CHECK
-- esperada, não de qualquer outro motivo.)
-- ---------------------------------------------------------------------
do $$
declare
  v_projeto_id uuid;
...
begin
  ...
  where exists (select 1 from public.usuarios u where u.empresa_id = p.empresa_id)
  limit 1;
  ...
  if v_projeto_id is null or v_aprovado_por is null then
    raise notice 'TESTE 8 PULADO: sem projeto + usuário no ambiente para inserir um snapshot de teste.';
  else
    begin
      insert into public.simulacoes_comerciais (
        empresa_id, projeto_id, cenario_demanda, modo_producao, data_necessidade,
        margem_seguranca_dias, janela_inicio, janela_fim, aprovado_por, vigente,
        ajuste_cenario, custo_adicional_total
      ) values (
        v_empresa_id, v_projeto_id, 'teste-fase6-check', 'teste-fase6-check', current_date,
        0, current_date, current_date + 30, v_aprovado_por, false,
        '{"versaoFormato": 1, "totais": {"custoAdicionalTotal": 500}}'::jsonb, 999
      );
      raise exception 'TESTE 8 FALHOU: INSERT com custo_adicional_total incoerente com ajuste_cenario.totais.custoAdicionalTotal deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 8 FALHOU%' then raise; end if;
        if sqlerrm not like '%ajuste_cenario_coerente%' then
          raise exception 'TESTE 8 FALHOU: erro inesperado, não é a CHECK esperada (simulacoes_comerciais_ajuste_cenario_coerente_chk) - %', sqlerrm;
        end if;
        raise notice 'TESTE 8 OK: incoerência ajuste_cenario × custo_adicional_total rejeitada pela CHECK esperada, via INSERT (não UPDATE) - (%).', sqlerrm;
    end;
  end if;
end;
$$;
