-- TESTE 4/4 da migration 202608110001_cenarios_schema_fase6.sql -
-- INTEGRAÇÃO FINAL - RPCs v4/v5. NÃO É UMA MIGRATION - vive em supabase/tests/, fora do
-- padrão de nome de migration, NUNCA deve ser aplicado em produção.
--
-- Escopo deste script: chamada real às RPCs aprovar_projeto_com_simulacao_v5 e v4 (caminho de rollback) após o schema completo instalado - aprovação, idempotência, replay idêntico (sem nenhuma escrita) e replay conflitante (rejeitado). Valida a migration completa e integrada, não uma peça isolada.
--
...
-- =====================================================================
-- PARTE 2: CENÁRIOS DE TESTE - INTEGRAÇÃO FINAL (RPCs v4/v5)
-- =====================================================================

-- ---------------------------------------------------------------------
-- PRE-CHECK: confirma, via to_regprocedure (não por suposição), que as
-- assinaturas EXATAS de v5 e v4 instaladas por ESTA transação são as
-- mesmas que os Testes 12/13 abaixo vão chamar - se a migration for
-- alterada no futuro e esta lista não acompanhar, o pre-check falha
-- primeiro, com uma mensagem clara, em vez de um 42883 genérico lá na
-- frente escondendo qual parâmetro mudou.
-- ---------------------------------------------------------------------
do $$
declare
  v_sig_v5 text := 'public.aprovar_projeto_com_simulacao_v5(uuid, uuid, text, text, date, integer, date, date, date, date, date, text, smallint, integer, jsonb, text, text)';
  v_sig_v4 text := 'public.aprovar_projeto_com_simulacao_v4(uuid, uuid, text, text, date, integer, date, date, date, date, jsonb, text, text)';
begin
  if to_regprocedure(v_sig_v5) is null then
    raise exception 'PRE-CHECK FALHOU: assinatura esperada da v5 não encontrada dentro desta transação - % - a migration mudou a assinatura e os Testes 12/13 (e/ou este pre-check) precisam ser atualizados juntos, não separadamente.', v_sig_v5;
  end if;
  if to_regprocedure(v_sig_v4) is null then
    raise exception 'PRE-CHECK FALHOU: assinatura esperada da v4 não encontrada dentro desta transação - % - a migration mudou a assinatura e os Testes 12/13 (e/ou este pre-check) precisam ser atualizados juntos, não separadamente.', v_sig_v4;
  end if;
  raise notice 'PRE-CHECK OK: assinaturas de aprovar_projeto_com_simulacao_v5 (17 parâmetros) e v4 (13 parâmetros) confirmadas via to_regprocedure, exatamente como declaradas nesta migration.';
end;
$$;


-- ---------------------------------------------------------------------
-- TESTE 12: regressão de ponta a ponta chamando a RPC REAL
-- aprovar_projeto_com_simulacao_v5 após o schema desta migration
-- instalado (a auditoria apontou que esse era exatamente o teste que
-- revelaria o P0: o trigger impedir_alteracao_vigente_sem_marca
-- quebrando a v5 original). Cobre: (1) snapshot novo nasce vigente=true
-- e o antigo é desativado; (2) replay idêntico (mesma chave_idempotencia
-- e hash) não escreve NADA - reforçado pela auditoria: além de vigente
-- continuar true, confere contagem de linhas em simulacoes_comerciais/
-- simulacao_comercial_itens/simulacao_comercial_item_distribuicoes
-- antes×depois (idênticas) e aprovado_em do próprio snapshot
-- (inalterado) - prova que o replay não é só "não mudou vigente", é
-- "não escreveu literalmente nada"; (3) replay conflitante (mesma
-- chave, hash diferente) é rejeitado sem escrita.
-- ---------------------------------------------------------------------
do $$
declare
  v_projeto_id uuid;
  v_empresa_id uuid;
  v_aprovado_por uuid;
  v_bom_operacao_id uuid;
  v_recurso_id uuid;
  v_vigente_antes uuid;
  v_snapshot_novo uuid;
  v_snapshot_replay uuid;
  v_itens jsonb;
  v_chave text := 'teste-fase6-v5-regressao-' || gen_random_uuid()::text;
  v_count_snapshots_antes integer;
  v_count_snapshots_depois integer;
  v_count_itens_antes integer;
  v_count_itens_depois integer;
  v_count_dist_antes integer;
  v_count_dist_depois integer;
  v_aprovado_em_antes timestamptz;
  v_aprovado_em_depois timestamptz;
begin
  select p.id, p.empresa_id into v_projeto_id, v_empresa_id
  from public.projetos p
  where exists (
      select 1 from public.bom_operacoes bo
      where bo.empresa_id = p.empresa_id and bo.ativo = true and bo.deleted_at is null
    )
    and exists (select 1 from public.recursos_produtivos rp where rp.empresa_id = p.empresa_id)
    and exists (select 1 from public.usuarios u where u.empresa_id = p.empresa_id)
  limit 1;

  if v_projeto_id is not null then
    select id into v_aprovado_por from public.usuarios where empresa_id = v_empresa_id limit 1;
    select id into v_bom_operacao_id from public.bom_operacoes
      where empresa_id = v_empresa_id and ativo = true and deleted_at is null limit 1;
    select id into v_recurso_id from public.recursos_produtivos where empresa_id = v_empresa_id limit 1;
  end if;

  if v_projeto_id is null or v_aprovado_por is null or v_bom_operacao_id is null or v_recurso_id is null then
    raise notice 'TESTE 12 PULADO: sem combinação de projeto + usuário + bom_operacao ativa + recurso, todos da mesma empresa, para chamar aprovar_projeto_com_simulacao_v5 de ponta a ponta.';
  else
    select id into v_vigente_antes from public.simulacoes_comerciais where projeto_id = v_projeto_id and vigente = true;

    -- Números internamente coerentes com TODAS as validações aritméticas
    -- da v5 (capacidade_efetiva = bruta×produtividade, disponível_inicial
    -- = max(0,efetiva-comprometido), disponível_depois = antes-alocadas,
    -- horas_maquina = alocadas/produtividade, soma+deficit=necessario) -
    -- não dependem dos valores REAIS cadastrados no recurso escolhido,
    -- só da coerência interna do payload, exatamente como a v5 valida.
    v_itens := jsonb_build_array(jsonb_build_object(
      'bom_operacao_id', v_bom_operacao_id,
      'recurso_original_id', v_recurso_id,
      'necessario', 10,
      'deficit', 0,
    ...
    -- 1ª chamada: aprovação real - novo snapshot deve nascer vigente=true,
    -- o antigo (se havia) deve ser desativado. Todo argumento com CAST
    -- explícito para o tipo exato do parâmetro correspondente na
    -- assinatura confirmada pelo PRE-CHECK acima - elimina qualquer
    -- ambiguidade de resolução de overload (ex.: literal inteiro contra
    -- parâmetro smallint), que já causou erro 42883 numa rodada anterior.
    v_snapshot_novo := public.aprovar_projeto_com_simulacao_v5(
      v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6'::text, 'sob_encomenda'::text, current_date::date,
      ...
      v_itens::jsonb, v_chave::text, 'hash-teste-fase6-v5'::text
    );

    if not exists (select 1 from public.simulacoes_comerciais where id = v_snapshot_novo and vigente = true) then
      raise exception 'TESTE 12 FALHOU: novo snapshot aprovado via v5 deveria estar vigente=true (compatibilização do P0 não funcionou).';
    end if;

    if v_vigente_antes is not null and exists (select 1 from public.simulacoes_comerciais where id = v_vigente_antes and vigente = true) then
      raise exception 'TESTE 12 FALHOU: snapshot antigo (%) deveria ter sido desativado pela v5.', v_vigente_antes;
    end if;

    -- Baseline ANTES do replay - contagens globais + aprovado_em do
    -- próprio snapshot, para provar que o replay não escreve NADA (não
    -- só "vigente continua true").
    select count(*) into v_count_snapshots_antes from public.simulacoes_comerciais;
    select count(*) into v_count_itens_antes from public.simulacao_comercial_itens;
    select count(*) into v_count_dist_antes from public.simulacao_comercial_item_distribuicoes;
    select aprovado_em into v_aprovado_em_antes from public.simulacoes_comerciais where id = v_snapshot_novo;

    -- 2ª chamada: replay EXATO (mesma chave_idempotencia e hash) - não
    -- pode alterar vigente nem criar linha nova.
    v_snapshot_replay := public.aprovar_projeto_com_simulacao_v5(
      v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6'::text, 'sob_encomenda'::text, current_date::date,
      0::integer, current_date::date, current_date::date,
      current_date::date, (current_date + 60)::date,
      (current_date + 10)::date, 'viavel'::text, 1::smallint, 5::integer,
    ...
    if v_snapshot_replay is distinct from v_snapshot_novo then
      raise exception 'TESTE 12 FALHOU: replay com a mesma chave_idempotencia deveria devolver o mesmo id (esperado %, obtido %).', v_snapshot_novo, v_snapshot_replay;
    end if;

    if not exists (select 1 from public.simulacoes_comerciais where id = v_snapshot_novo and vigente = true) then
      raise exception 'TESTE 12 FALHOU: replay idêntico não deveria ter alterado vigente do snapshot.';
    end if;

    select count(*) into v_count_snapshots_depois from public.simulacoes_comerciais;
    select count(*) into v_count_itens_depois from public.simulacao_comercial_itens;
    select count(*) into v_count_dist_depois from public.simulacao_comercial_item_distribuicoes;
    select aprovado_em into v_aprovado_em_depois from public.simulacoes_comerciais where id = v_snapshot_novo;

    if v_count_snapshots_depois <> v_count_snapshots_antes
       or v_count_itens_depois <> v_count_itens_antes
       or v_count_dist_depois <> v_count_dist_antes
       or v_aprovado_em_depois is distinct from v_aprovado_em_antes then
      raise exception 'TESTE 12 FALHOU: replay idêntico deveria não ter escrito NADA - snapshots %→%, itens %→%, distribuições %→%, aprovado_em %→%.',
        v_count_snapshots_antes, v_count_snapshots_depois, v_count_itens_antes, v_count_itens_depois,
        v_count_dist_antes, v_count_dist_depois, v_aprovado_em_antes, v_aprovado_em_depois;
    end if;

    -- 3ª chamada: mesma chave, hash DIFERENTE - conflito, deve ser
    -- rejeitado, sem nenhuma escrita.
    begin
      perform public.aprovar_projeto_com_simulacao_v5(
        v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6'::text, 'sob_encomenda'::text, current_date::date,
        0::integer, current_date::date, current_date::date,
        current_date::date, (current_date + 60)::date,
        (current_date + 10)::date, 'viavel'::text, 1::smallint, 5::integer,
        v_itens::jsonb, v_chave::text, 'hash-DIFERENTE'::text
      );
      raise exception 'TESTE 12 FALHOU: replay com hash diferente deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 12 FALHOU%' then raise; end if;
        raise notice 'TESTE 12 OK (parte 3): replay conflitante (chave igual, hash diferente) rejeitado (%).', sqlerrm;
    end;

    raise notice 'TESTE 12 OK: aprovar_projeto_com_simulacao_v5 funciona com o novo trigger de vigente (compatibilização do P0, Fase 6) - snapshot novo vigente=true, antigo desativado, replay idêntico sem NENHUMA escrita (contagens e aprovado_em confirmados), replay conflitante rejeitado.';
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 13: regressão de ponta a ponta chamando a RPC REAL
-- aprovar_projeto_com_simulacao_v4 (caminho de ROLLBACK) após o schema
-- desta migration instalado (NOVO - auditoria apontou que a v4 tem
-- exatamente o mesmo problema estrutural que a v5 tinha: insere
-- vigente=false e ativa via UPDATE, sem marca - o trigger bloquearia a
-- v4 também, tornando o "caminho de rollback" documentado inutilizável
-- exatamente quando precisaria funcionar). Mesma cobertura do TESTE 12,
-- adaptada para a assinatura de 13 parâmetros da v4 (sem os 4 campos de
-- estimativa, exclusivos da v5).
-- ---------------------------------------------------------------------
do $$
declare
  v_projeto_id uuid;
  ...
  v_snapshot_replay uuid;
  v_itens jsonb;
  v_chave text := 'teste-fase6-v4-regressao-' || gen_random_uuid()::text;
  v_count_snapshots_antes integer;
  v_count_snapshots_depois integer;
  v_aprovado_em_antes timestamptz;
  v_aprovado_em_depois timestamptz;
begin
  ...
  if v_projeto_id is null or v_aprovado_por is null or v_bom_operacao_id is null or v_recurso_id is null then
    raise notice 'TESTE 13 PULADO: sem combinação de projeto + usuário + bom_operacao ativa + recurso, todos da mesma empresa, para chamar aprovar_projeto_com_simulacao_v4 de ponta a ponta.';
  else
    select id into v_vigente_antes from public.simulacoes_comerciais where projeto_id = v_projeto_id and vigente = true;
    ...
    -- 1ª chamada: aprovação real via v4 (rollback) - novo snapshot deve
    -- nascer vigente=true, o antigo (se havia) deve ser desativado. Todo
    -- argumento com CAST explícito para o tipo exato do parâmetro
    -- correspondente na assinatura confirmada pelo PRE-CHECK acima.
    v_snapshot_novo := public.aprovar_projeto_com_simulacao_v4(
      v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6-v4'::text, 'sob_encomenda'::text, current_date::date,
      0::integer, current_date::date, current_date::date,
      current_date::date, (current_date + 60)::date,
      v_itens::jsonb, v_chave::text, 'hash-teste-fase6-v4'::text
    );

    if not exists (select 1 from public.simulacoes_comerciais where id = v_snapshot_novo and vigente = true) then
      raise exception 'TESTE 13 FALHOU: novo snapshot aprovado via v4 (rollback) deveria estar vigente=true - caminho de rollback continua inutilizável.';
    end if;

    if v_vigente_antes is not null and exists (select 1 from public.simulacoes_comerciais where id = v_vigente_antes and vigente = true) then
      raise exception 'TESTE 13 FALHOU: snapshot antigo (%) deveria ter sido desativado pela v4.', v_vigente_antes;
    end if;

    select count(*) into v_count_snapshots_antes from public.simulacoes_comerciais;
    select aprovado_em into v_aprovado_em_antes from public.simulacoes_comerciais where id = v_snapshot_novo;

    -- 2ª chamada: replay EXATO - não pode escrever nada.
    v_snapshot_replay := public.aprovar_projeto_com_simulacao_v4(
      v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6-v4'::text, 'sob_encomenda'::text, current_date::date,
      0::integer, current_date::date, current_date::date,
      current_date::date, (current_date + 60)::date,
      v_itens::jsonb, v_chave::text, 'hash-teste-fase6-v4'::text
    ...
    if v_snapshot_replay is distinct from v_snapshot_novo then
      raise exception 'TESTE 13 FALHOU: replay com a mesma chave_idempotencia deveria devolver o mesmo id (esperado %, obtido %).', v_snapshot_novo, v_snapshot_replay;
    end if;

    select count(*) into v_count_snapshots_depois from public.simulacoes_comerciais;
    select aprovado_em into v_aprovado_em_depois from public.simulacoes_comerciais where id = v_snapshot_novo;

    if v_count_snapshots_depois <> v_count_snapshots_antes or v_aprovado_em_depois is distinct from v_aprovado_em_antes then
      raise exception 'TESTE 13 FALHOU: replay idêntico via v4 deveria não ter escrito NADA - snapshots %→%, aprovado_em %→%.',
        v_count_snapshots_antes, v_count_snapshots_depois, v_aprovado_em_antes, v_aprovado_em_depois;
    end if;

    -- 3ª chamada: mesma chave, hash DIFERENTE - conflito, deve ser rejeitado.
    begin
      perform public.aprovar_projeto_com_simulacao_v4(
        v_aprovado_por::uuid, v_projeto_id::uuid, 'teste-fase6-v4'::text, 'sob_encomenda'::text, current_date::date,
        0::integer, current_date::date, current_date::date,
        current_date::date, (current_date + 60)::date,
        v_itens::jsonb, v_chave::text, 'hash-DIFERENTE'::text
      );
      raise exception 'TESTE 13 FALHOU: replay com hash diferente deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 13 FALHOU%' then raise; end if;
        raise notice 'TESTE 13 OK (parte 3): replay conflitante (chave igual, hash diferente) rejeitado (%).', sqlerrm;
    end;

    raise notice 'TESTE 13 OK: aprovar_projeto_com_simulacao_v4 (caminho de ROLLBACK) funciona com o novo trigger de vigente - snapshot novo vigente=true, antigo desativado, replay idêntico sem escrita, replay conflitante rejeitado. O rollback documentado permanece utilizável.';
  end if;
end;
$$;
