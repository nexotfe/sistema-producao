-- TESTE 2/4 da migration 202608110001_cenarios_schema_fase6.sql -
-- PRECEDÊNCIA E VERSIONAMENTO. NÃO É UMA MIGRATION - vive em supabase/tests/, fora do
-- padrão de nome de migration, NUNCA deve ser aplicado em produção.
--
-- Escopo deste script: validação de dependência de subconjunto (mesmo bom_id, componente_tipo, ativos, FK composta cross-tenant) e triggers de incremento de versão (escopo por empresa, exceção global de calendario_oficial_feriados, inventário nominal de 19 triggers).
--
...
-- =====================================================================
-- PARTE 2: CENÁRIOS DE TESTE - PRECEDÊNCIA E VERSIONAMENTO
-- =====================================================================

-- ---------------------------------------------------------------------
-- TESTE 5: bom_operacao_dependencias_subconjunto - validações de campo
-- (corrigido 2x pela auditoria: 1ª rodada - o teste aceitava "por
-- acaso"; 2ª rodada - a seleção ainda era arbitrária, escolhendo a
-- operação PRIMEIRO e só depois procurando item no bom_id dela, o que
-- podia pular candidatos válidos mesmo havendo outro bom adequado no
-- ambiente. Agora operação+item são selecionados JUNTOS por JOIN,
-- percorrendo TODOS os bom_operacoes/bom_itens do ambiente de uma vez -
-- 5a e 5b são buscas independentes, cada uma cobrindo todo o ambiente.)
-- ---------------------------------------------------------------------
do $$
declare
  v_op_id_a uuid;
  v_op_empresa_a uuid;
  v_item_nao_subconjunto uuid;
  v_op_id_b uuid;
  v_op_empresa_b uuid;
  v_item_subconjunto uuid;
begin
  -- 5a: par (operação, item) do MESMO bom_id, item com componente_tipo
  -- <> subconjunto - JOIN sobre TODOS os bom_operacoes/bom_itens ativos
  -- do ambiente, não só o bom_id da primeira operação encontrada.
  select bo.id, bo.empresa_id, bi.id
    into v_op_id_a, v_op_empresa_a, v_item_nao_subconjunto
  from public.bom_operacoes bo
  join public.bom_itens bi on bi.bom_id = bo.bom_id and bi.empresa_id = bo.empresa_id
  where bo.ativo = true and bo.deleted_at is null
    and bi.ativo = true and bi.deleted_at is null
    and bi.componente_tipo <> 'subconjunto'
  limit 1;

  if v_op_id_a is null then
    raise notice 'TESTE 5a PULADO: nenhum par (bom_operacoes ativa, bom_itens não-subconjunto ativo) no MESMO bom_id em todo o ambiente.';
  else
    begin
      insert into public.bom_operacao_dependencias_subconjunto (empresa_id, bom_operacao_id, bom_item_id)
      values (v_op_empresa_a, v_op_id_a, v_item_nao_subconjunto);
      raise exception 'TESTE 5a FALHOU: vínculo com item que não é subconjunto deveria ter sido rejeitado.';
    exception
      when others then
        if sqlerrm like 'TESTE 5a FALHOU%' then raise; end if;
        raise notice 'TESTE 5a OK: item não-subconjunto rejeitado deterministicamente (%).', sqlerrm;
    end;
  end if;

  -- 5b: par (operação, item) do MESMO bom_id, item subconjunto REAL -
  -- busca INDEPENDENTE da 5a (também por JOIN sobre todo o ambiente),
  -- não reaproveita v_op_id_a/v_op_empresa_a.
  select bo.id, bo.empresa_id, bi.id
    into v_op_id_b, v_op_empresa_b, v_item_subconjunto
  from public.bom_operacoes bo
  join public.bom_itens bi on bi.bom_id = bo.bom_id and bi.empresa_id = bo.empresa_id
  where bo.ativo = true and bo.deleted_at is null
    and bi.ativo = true and bi.deleted_at is null
  ...
  if v_op_id_b is null then
    raise notice 'TESTE 5b PULADO: nenhum par (bom_operacoes ativa, bom_itens subconjunto ativo) no MESMO bom_id em todo o ambiente.';
  else
    begin
      insert into public.bom_operacao_dependencias_subconjunto (empresa_id, bom_operacao_id, bom_item_id)
      values (v_op_empresa_b, v_op_id_b, v_item_subconjunto);
      raise notice 'TESTE 5b OK: vínculo coerente (mesmo bom_id, subconjunto real) aceito normalmente.';
    exception
      when others then
        raise exception 'TESTE 5b FALHOU: vínculo coerente deveria ter sido aceito - %.', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 9: calendario_oficial_feriados incrementa a versão de TODAS as
-- empresas (caso especial, tabela global sem empresa_id)
-- ---------------------------------------------------------------------
do $$
declare
  v_versoes_antes bigint[];
  v_versoes_depois bigint[];
  v_todas_incrementaram boolean;
begin
  select array_agg(versao order by empresa_id) into v_versoes_antes from public.empresa_capacidade_versoes;

  insert into public.calendario_oficial_feriados (data, abrangencia, descricao)
  values ('2099-12-25', 'nacional', 'Feriado de teste Fase 6 (rollback)');

  select array_agg(versao order by empresa_id) into v_versoes_depois from public.empresa_capacidade_versoes;

  select bool_and(depois > antes) into v_todas_incrementaram
  from unnest(v_versoes_antes) with ordinality as a(antes, ord)
  join unnest(v_versoes_depois) with ordinality as d(depois, ord) using (ord);

  if not coalesce(v_todas_incrementaram, false) then
    raise exception 'TESTE 9 FALHOU: inserir em calendario_oficial_feriados deveria incrementar a versão de TODAS as empresas.';
  end if;
  raise notice 'TESTE 9 OK: feriado global incrementou a versão de todas as % empresas.', array_length(v_versoes_antes, 1);
end;
$$;


-- ---------------------------------------------------------------------
-- TESTE 10: incremento de versão escopado por empresa_id não afeta
-- outras empresas (caso comum do inventário §14)
-- (corrigido pela auditoria: o teste original comparava a versão de
-- cada OUTRA empresa contra ELA MESMA - sempre igual, nunca provava
-- nada - e nunca usava o resultado na asserção final. Agora captura
-- antes/depois de TODAS as outras empresas, na mesma ordem, e compara
-- os dois arrays.)
-- ---------------------------------------------------------------------
do $$
declare
  v_empresa_alvo uuid;
  v_versao_alvo_antes bigint;
  v_versao_alvo_depois bigint;
  v_outras_antes bigint[];
  v_outras_depois bigint[];
begin
  select empresa_id into v_empresa_alvo from public.projetos limit 1;
  if v_empresa_alvo is null then
    raise notice 'TESTE 10 PULADO: nenhum projeto no ambiente.';
  else
    select versao into v_versao_alvo_antes from public.empresa_capacidade_versoes where empresa_id = v_empresa_alvo;
    select array_agg(versao order by empresa_id) into v_outras_antes
      from public.empresa_capacidade_versoes where empresa_id <> v_empresa_alvo;

    update public.projetos set prioridade = prioridade where empresa_id = v_empresa_alvo;

    select versao into v_versao_alvo_depois from public.empresa_capacidade_versoes where empresa_id = v_empresa_alvo;
    select array_agg(versao order by empresa_id) into v_outras_depois
      from public.empresa_capacidade_versoes where empresa_id <> v_empresa_alvo;

    if v_versao_alvo_depois <= v_versao_alvo_antes then
      raise exception 'TESTE 10 FALHOU: versão da empresa alvo deveria ter incrementado (antes=%, depois=%).', v_versao_alvo_antes, v_versao_alvo_depois;
    end if;

    if v_outras_antes is distinct from v_outras_depois then
      raise exception 'TESTE 10 FALHOU: versão de OUTRA empresa mudou quando só a empresa alvo deveria ter mudado (antes=%, depois=%).', v_outras_antes, v_outras_depois;
    end if;

    raise notice 'TESTE 10 OK: só a empresa alvo teve a versão incrementada (antes=%, depois=%); as demais % empresa(s) permaneceram idênticas.', v_versao_alvo_antes, v_versao_alvo_depois, coalesce(array_length(v_outras_antes, 1), 0);
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- TESTE 11: FK composta rejeita vínculo entre empresas diferentes
-- (bom_operacao_dependencias_subconjunto)
-- ---------------------------------------------------------------------
do $$
declare
  v_op_id uuid;
  v_item_outra_empresa uuid;
  v_empresa_a uuid;
  v_empresa_b uuid;
begin
  select id, empresa_id into v_op_id, v_empresa_a from public.bom_operacoes limit 1;
  select bi.id into v_item_outra_empresa
    from public.bom_itens bi
    where bi.componente_tipo = 'subconjunto' and bi.empresa_id <> v_empresa_a
    limit 1;

  if v_op_id is null or v_item_outra_empresa is null then
    raise notice 'TESTE 11 PULADO: sem fixture de 2 empresas diferentes com bom_operacoes/bom_itens subconjunto para testar isolamento cross-tenant.';
  else
    begin
      insert into public.bom_operacao_dependencias_subconjunto (empresa_id, bom_operacao_id, bom_item_id)
      values (v_empresa_a, v_op_id, v_item_outra_empresa); -- bom_item_id pertence a OUTRA empresa
      raise exception 'TESTE 11 FALHOU: vínculo entre operação de uma empresa e item de outra deveria ter sido rejeitado pela FK composta.';
    exception
      when others then
        if sqlerrm like 'TESTE 11 FALHOU%' then raise; end if;
        raise notice 'TESTE 11 OK: FK composta rejeitou vínculo cross-tenant (%).', sqlerrm;
    end;
  end if;
end;
$$;
...
-- ---------------------------------------------------------------------
-- Resumo: contagem de triggers de versão criados (deve ser 20 - inventário nominal completo)
-- ---------------------------------------------------------------------
do $$
declare
  v_total integer;
begin
  ...
  where p.proname in ('trg_bump_capacidade_versao_por_empresa_id', 'trg_bump_capacidade_versao_todas_empresas')
    and not t.tgisinternal;

  if v_total <> 19 then
    raise exception 'RESUMO FALHOU: esperado 19 triggers de incremento de versão, encontrado %.', v_total;
  end if;
  raise notice 'RESUMO OK: % triggers de incremento de versão criados (inventário nominal completo).', v_total;
end;
$$;
